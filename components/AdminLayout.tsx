...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-primary-50">
      <nav className="bg-white shadow-md border-b-2 border-[#051A53] sticky top-0 z-50">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-4 md:space-x-8">
              <Link href="/admin/dashboard" className="text-lg md:text-xl font-bold text-[#051A53] hover:text-[#051A53] transition-colors">
                Theta Admin
              </Link>
                ...
                {navItems.map((item) => (
                  <Link
                    ...
                    className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                      pathname === item.href
                        ? 'bg-[#051A53] text-white shadow-md'
                        : 'text-gray-700 hover:bg-[#051A53] hover:text-white'
                    }`}
                  >
                    {item.label}
                  </Link>
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              ...
              <Link
                href="/admin/quotes"
                className="relative p-2 text-gray-700 hover:text-[#051A53] transition-colors"
              >
                <svg
              ...
              <button
                ...
                className="hidden md:block px-4 py-2 text-sm font-medium text-white bg-[#051A53] hover:bg-[#051A53] rounded-lg transition-colors"
              >
                {t('logout')}
              </button>
            </div>
          ...
          {mobileMenuOpen && (
            <div className="md:hidden border-t py-4 space-y-2">
              ...
              <button
                ...
                className="block w-full text-left px-3 py-2 rounded-md text-sm font-medium text-white bg-[#051A53] hover:bg-[#051A53] transition-colors"
              >
                {t('logout')}
              </button>
            </div>
